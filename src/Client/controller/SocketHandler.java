...
public class SocketHandler {
    ...
    private void onReceiveCreateRoom(String received) {
        ...
        if (status.equals("failed")) {
            ...
            // change scene and request room data on EDT
    ...
    private void onReceiveDataRoom(String received) {
        ...
        if (status.equals("failed")) {
            ...
            // capture history points
            ArrayList<int[]> historyPoints = new ArrayList<>();
            ArrayList<String> historyEmails = new ArrayList<>();
            for (int i = 0; i < historyCount; i++) {
                int r = Integer.parseInt(splitted[index++]);
                int c = Integer.parseInt(splitted[index++]);
                String e = splitted[index++];
                historyPoints.add(new int[]{r, c});
                historyEmails.add(e);
            }

            // last email is at splitted[index - 1]
            String lastEmail = splitted[index - 1];
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                for (int i = 0; i < historyPoints.size(); i++) {
                    int[] pt = historyPoints.get(i);
                    RunClient.inGameScene.addPoint(pt[0], pt[1], historyEmails.get(i));
                }

                // change turn
                RunClient.inGameScene.changeTurnFrom(lastEmail);
            });
        }
    }
    ...
    public void joinRoom(String roomId) {
        sendData(StreamData.Type.JOIN_ROOM.name() + ";" + roomId);
    }
