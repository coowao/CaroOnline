...
public class Client implements Runnable {
    ...
    private void onReceiveCreateRoom(String received) {
        // nếu đang ở trong phòng rồi thì không thể tạo 
        if (this.joinedRoom != null) {
            sendData(StreamData.Type.CREATE_ROOM.name() + ";failed;" + Code.ALREADY_INROOM + " #" + this.joinedRoom.getId());
            return;
        }
        ...
        // join vào phòng vừa tạo (là người chơi, không phải watcher)
        String status = this.joinRoom(newRoom, false);

        if (status.startsWith("success")) {
            // trả về id phòng cho client
            sendData(StreamData.Type.CREATE_ROOM.name() + ";success;" + newRoom.getId());
        } else {
            // trả về lỗi nếu có
            sendData(StreamData.Type.CREATE_ROOM.name() + ";" + status);
        }
    }
    ...
    private void onReceiveJoinRoom(String received) {
        try {
            ...
            String status = joinRoom(roomId, false);

            sendData(StreamData.Type.JOIN_ROOM.name() + ";" + status);
        } catch (Exception e) {
            sendData(StreamData.Type.JOIN_ROOM.name() + ";failed;Lỗi khi xử lý JOIN_ROOM");
        }
    }
    ...
    public String joinRoom(Room room, boolean isWatcher) {
        ...
        if (room.addClient(this, isWatcher)) {
            ...
            // gửi dữ liệu phòng cập nhật cho tất cả client trong phòng
            this.joinedRoom.broadcast(StreamData.Type.DATA_ROOM.name() + ";success;" + this.joinedRoom.getFullData());
